"""Birchwood pilot success metrics + cost estimates (PR 5).

Core computation lives here in src/ so production code (e.g. the
enrichment insights feature) can import it — the Docker image ships src/
but not scripts/. The operator CLI in scripts/pilot_metrics.py is a thin
