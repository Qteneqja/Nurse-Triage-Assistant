...
class DeepSeekClient:
    ...
    def __init__(self):
        ...
        self.client = AsyncOpenAI(
            base_url=DEEPSEEK_BASE_URL, api_key=DEEPSEEK_API_KEY, timeout=LLM_TIMEOUT
        )
    ...
    async def get_triage_decision(
        self, conversation_history: list[dict], patient_facts: Optional[dict] = None
    ) -> TriageResult:
        """
        ...
        try:
            ...
            if question_count >= MAX_QUESTIONS:
                logger.warning(
                    f"[TRIAGE] MAX_QUESTIONS ({MAX_QUESTIONS}) exceeded, forcing stop"
                )
                return TriageResult(
            ...
            chief_complaint = (
                patient_facts.get("chief_complaint") if patient_facts else None
            )
            if chief_complaint:
                logger.info(
                    f"[TRIAGE] Using chief complaint for natural questions: '{chief_complaint}'"
                )
                messages.append(
                    {
                        "role": "system",
            ...
            if json_match:
                try:
                    ...
                    logger.info(
                        f"[TRIAGE] Complaint category: {data.get('complaint_category', 'N/A')}"
                    )
                    logger.info(
                        f"[TRIAGE] Covered items: {data.get('covered_items', [])}"
                    )
                    logger.info(
                        f"[TRIAGE] Remaining items: {data.get('remaining_items', [])}"
                    )
                    logger.info(
                        f"[TRIAGE] Target item: {data.get('target_item', 'N/A')}"
                    )
                    ...
                    if should_stop:
                        ...
                        logger.info(
                            f"[TRIAGE] STOPPING - Disposition: {disposition.value}"
                        )
                        ...
                        return TriageResult(
                            ...
                            rationale_bullets=[
                                data.get("final_message", "Triage complete")
                            ]
                        ...
                        if not next_question:
                            logger.warning(
                                "[TRIAGE] No next_question in JSON, using fallback"
                            )
                            return self._fallback_result(
                                conversation_history, question_count
                            )

                        logger.info(
                            f"[TRIAGE] CONTINUING - Next question: {next_question[:100]}"
                        )
            ...
            return self._fallback_result(
                ...
                question_count if "question_count" in locals() else 0,
            )

    async def _attempt_json_repair(
        self, messages: list, bad_content: str
...
Return ONLY JSON. No markdown. No text. Just the JSON object."""
            ...
            if json_match:
                ...
                if should_stop:
                    ...
                    disposition = (
                        DispositionType[disposition_str]
                        if disposition_str in DispositionType.__members__
                        else DispositionType.HUMAN_REVIEW
                    )

                    final_message = data.get("final_message", "")
                    logger.info(
                        f"[TRIAGE] Repair successful with final message: {final_message[:100]}..."
                    )
                    ...
                    return TriageResult(
                        ...
                        rationale_bullets=[final_message]
                        if final_message
                        else ["Triage complete"],
                        symptoms=[],
                    )
                    ...
                    if next_question:
                        logger.info(
                            f"[TRIAGE] Repair successful: {next_question[:100]}"
                        )
                        return TriageResult(
                            triage_disposition=DispositionType.HUMAN_REVIEW,
                            confidence=0.8,
    ...
    def _extract_chief_complaint(
        ...
        for msg in conversation_history:
            if msg.get("role") == "user":
                ...
                if len(content) > 5 and content.lower() not in [
                    "yes",
                    "no",
                    "yeah",
                    "nope",
    ...
    def _fallback_result(
        self, conversation_history: list[dict], question_count: int = 0
    ) -> TriageResult:
        ...
        if question_count >= MAX_QUESTIONS - 1:
            logger.warning(
                "[TRIAGE] Fallback triggered near MAX_QUESTIONS, forcing stop"
            )
            return TriageResult(
                triage_disposition=DispositionType.HUMAN_REVIEW,
                confidence=0.7,
    ...
    async def generate_patient_summary(
        self, triage_result: TriageResult, patient_facts: dict, recent_snippet: str
    ) -> str:
        """
...
Message for patient:"""
    ...
    def _default_patient_message(self, disposition: DispositionType) -> str:
        ...
        return messages.get(
            disposition,
            "Thank you for using our triage service. Please follow up with a healthcare provider as needed.",
        )
...
Be concise and clinical. Only include information that was actually discussed. Use 'not assessed' for missing data."""
            ...
            sbar_text = (
                sbar_response.choices[0].message.content or "SBAR generation failed"
            )
...
Use null or 'unknown' for data not collected. No hallucinations."""
            ...
            if json_match:
                ...
                structured_data = {
                    ...
                    "disposition": {
                        "level": triage_result.triage_disposition.value,
                        "rationale": ", ".join(triage_result.rationale_bullets),
                    },
                }
            ...
            return {
                ...
                "structured": {
                    ...
                    "chief_complaint": session_metadata.get(
                        "chief_complaint", "Not documented"
                    ),
                    "disposition": {
                        "level": triage_result.triage_disposition.value,
                        "rationale": "Error in report generation",
                    },
                },
                "session_id": session_id,
            }
