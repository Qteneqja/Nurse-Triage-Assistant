...
_FIRST = ["Jordan", "Casey", "Avery", "Riley", "Morgan", "Quinn", "Dana",
          "Jamie", "Alexis", "Robin", "Sam", "Taylor", "Drew", "Reese",
          "Skyler", "Parker"]
_LAST = ["Sample", "Demo", "Testfield", "Placeholder", "Mockley", "Specimen",
         "Example", "Stand-in"]

_VEHICLES = [
    (2021, "Toyota", "Corolla"), (2019, "Honda", "Civic"),
    (2022, "Ford", "F-150"), (2018, "Mazda", "CX-5"),
    (2023, "Hyundai", "Tucson"), (2017, "Chevrolet", "Equinox"),
    (2020, "Kia", "Sportage"), (2016, "Dodge", "Grand Caravan"),
    (2022, "Subaru", "Outback"), (2021, "Nissan", "Rogue"),
    (2024, "Toyota", "RAV4"), (2015, "Honda", "CR-V"),
    (2023, "GMC", "Sierra"), (2019, "Volkswagen", "Tiguan"),
    (2020, "Jeep", "Wrangler"), (2022, "BMW", "X3"),
    (2018, "Ford", "Escape"), (2021, "Ram", "1500"),
]
...
_INSURERS = ["MPI", "MPI", "MPI", "MPI", "MPI", "MPI", "MPI",
             "Wawanesa", "Wawanesa", "Intact", "Aviva", "SGI"]

_LOCATIONS = ["Pembina and Stafford", "Portage and Main", "Kenaston and McGillivray",
              "Regent and Lagimodiere", "Henderson and Chief Peguis", "Route 90 near Ness",
              "McPhillips and Leila", "Bishop Grandin and St. Mary's", "Osborne Village",
              "Corydon and Stafford", "Inkster and Keewatin", "Fermor and St. Anne's"]

_OTHER_PARTIES = ["another vehicle involved", "two-vehicle collision",
                  "hit a parked car", "hit and run - other driver left",
                  "single vehicle, hit a pole", "another vehicle involved"]

# Weighted call-hour distribution: morning and early-afternoon peaks,
# a few early-evening calls. (hour, weight)
_HOURS = [(8, 3), (9, 5), (10, 4), (11, 3), (12, 2), (13, 4), (14, 4),
          (15, 3), (16, 2), (17, 2), (18, 1), (19, 1)]

_STATUS_PLAN = (["new"] * 11 + ["contacted"] * 8 + ["scheduled"] * 7
                + ["completed"] * 6)  # 32 records
...
def _turns(name: str, vehicle: str, damage: str, drivable: bool,
           injured: bool, insurer: str | None) -> list[ConversationTurn]:
    drive_text = (
        "Yes, it still drives fine." if drivable
        else "No - it had to be towed, it's not drivable."
    )
    injury_text = (
        "My neck has been a bit sore since it happened."
        if injured else "No, nobody was hurt."
    )
    insurer_text = (
        f"I'm going through {insurer}." if insurer else "I'll be paying privately."
    )
    script = [
        ("assistant", "Thanks for calling Birchwood Collision. I can get your "
                      "intake started - this'll take about two minutes."),
        ("caller", f"I was in an accident with my {vehicle}. The {damage} "
...
def _build_session(index: int, run_tag: str) -> tuple[OrchestratorSession, str]:
    ...
    claim_number = (
        None if private_pay or claim_missing
    ...
    if injured:
        disposition = "INCOMPLETE_CALLBACK_NEEDED" if callback_needed else "COMPLETED_INTAKE"
    elif transfer:
    ...
    incident_when = ["this morning", "yesterday afternoon", "last night",
                     "two days ago", "earlier today"][index % 5]
    ...
    record = {
        ...
        "vehicle": {
            "year": year, "make": make, "model": model, "license_plate": None,
            "is_luxury": make in ("BMW",), "is_rebuilt": False,
            "is_rebuilt_or_salvage": False, "is_drivable": drivable,
        },
        "incident": {
            "damage_type": damage, "damage_area": area, "is_drivable": drivable,
            "description": narrative, "incident_datetime": incident_when,
            "incident_location": location,
            "injuries_state": "reported" if injured else "denied",
            "other_parties": other, "police_report_filed": police,
            "photos_available": photos, "glass_only": glass_only,
            "body_damage": not glass_only,
        },
        "insurance": {
            "filing_claim": not private_pay,
            "filing_insurance_claim": not private_pay,
            "insurance_provider": insurer, "claim_number": claim_number,
        ...
        "caller_name": name, "phone": phone, "email": None, "address": None,
        "vehicle_year": year, "vehicle_make": make, "vehicle_model": model,
        "license_plate": None, "is_drivable": drivable, "damage_type": damage,
        "damage_area": area, "glass_only": glass_only,
        "body_damage": not glass_only, "incident_description": narrative,
        "incident_datetime": incident_when, "incident_location": location,
        "injuries_state": "reported" if injured else "denied",
        "other_parties": other, "police_report_filed": police,
        "photos_available": photos, "filing_insurance_claim": not private_pay,
        "insurance_provider": insurer, "claim_number": claim_number,
        "private_pay": private_pay,
        "preferred_collision_center": f"BIRCHWOOD_COLLISION_LOCATION_{index % 3 + 1}",
        "preferred_timing": ["this week", "as soon as possible", "next week"][index % 3],
    ...
    session.conversation = _turns(name, vehicle_label, damage, drivable,
                                  injured, insurer)
...
def seed_records(count: int = 32, run_tag: str | None = None) -> list[str]:
    ...
    for index in range(count):
        ...
        steps = {"new": [], "contacted": ["contacted"],
                 "scheduled": ["contacted", "scheduled"],
                 "completed": ["contacted", "scheduled", "completed"]}[target_status]
        notes = {"contacted": "Reached the customer, going over options.",
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("--remove", action="store_true",
                        help="remove all seeded pitch records")
    ...
    print("Open /dashboard/birchwood — remove later with: "
