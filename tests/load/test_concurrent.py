...
class TestConcurrentSessions:
    ...
    async def test_10_concurrent_sessions(
        ...
        # Run concurrently
        tasks = [
            process_session(f"concurrent-{i:03d}") for i in range(num_sessions)
        ]
    ...
    async def test_50_concurrent_sessions(
        ...
        # Run concurrently
        tasks = [
            process_session(f"heavy-{i:03d}") for i in range(num_sessions)
        ]
        results = await asyncio.gather(*tasks, return_exceptions=True)
        
        # Should handle scaling
        successful = sum(1 for r in results if r is True or
...
class TestSessionIsolation:
    ...
    async def test_session_data_isolation(
        ...
        tasks = [
            session_task(sid, name, age) for sid, name, age in sessions_info
        ]
    ...
    async def test_transcript_list_performance(
        ...
        for i in range(500):
            mock_storage.save_transcript(
                session_id,
                {"turn": i, "text": f"Turn {i} transcript"}
            )
        elapsed = time.time() - start
