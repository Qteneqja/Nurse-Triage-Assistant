...
class TestCanonicalEnumEnforcement:
    ...
    def test_canonical_values(self):
        from src.shared.canonical import (
            CanonicalDisposition,
            CANONICAL_DISPOSITION_VALUES,
        )
    ...
    def test_disposition_category_uses_canonical(self):
        ...
        assert values == expected, (
            f"DispositionCategory has non-canonical values: {values - expected}"
        )
    ...
    def test_phase1_disposition_uses_canonical(self):
        ...
        expected = {"ER_NOW", "URGENT", "SCHEDULE", "SELF_CARE", "HUMAN_REVIEW"}
        assert values == expected, (
            f"Phase1Disposition has non-canonical values: {values - expected}"
        )
...
class TestNoDirectLLMInOrchestrator:
    ...
    def test_no_structured_client_import(self):
        ...
        assert (
            "from src.llm.client import" not in text
            or "LLMCallError"
            in text.split("from src.llm.client import")[1].split("\n")[0]
